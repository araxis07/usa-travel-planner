...
function render(lang, state, index) {
  ...
  const links = state
    ...
    : catalog.states
        ...
        .join('');
  const content = `<header><a href="/${lang}/">Roam America</a><nav style="font-family:system-ui,sans-serif">${langs.map((code, i) => `<a lang="${code}" href="${pagePath(code, state, index)}">${labels[i]}</a>`).join(' · ')}</nav></header><main style="max-width:1000px;margin:auto;padding:32px"><h1>${escape(name)}</h1>${photo ? `<img src="${photo.src}" alt="${escape(name)}" width="960" style="max-width:100%;height:auto"/>` : ''}<p>${escape(description)}</p>${profile ? `${profile.advisory ? `<aside><p>${escape(profile.advisory.text[l])}</p><a href="${escape(profile.advisory.source)}">${escape(profile.advisory.checkedAt)}</a></aside>` : ''}<p>${escape(profile.access[l])}</p><p>${escape(profile.stay[l])}</p><a href="${escape(profile.officialUrl)}">${escape(profile.officialUrl)}</a><p><a href="${escape(profile.summarySources[l])}">Wikipedia contributors</a> · <a href="https://creativecommons.org/licenses/by-sa/4.0/">${escape(profile.summaryLicense)}</a></p>` : ''}<ul>${links}</ul></main>`;
  const json = state
  ...
  const html = template
    ...
    .replace(/<link\s+data-roam-mobile-cover\s[^>]+>/, (tag) => (photo ? '' : tag))
